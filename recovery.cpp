...
Device::BuiltinAction start_recovery(Device* device, const std::vector<std::string>& args) {
  ...
  std::string ver = android::base::GetProperty("ro.build.version.incremental", "");
  std::smatch ver_date_match;
  std::regex_search(ver, ver_date_match, std::regex("\\.(\\d{8}(\\.\\d{6})?)\\."));
  std::string ver_date = ver_date_match.str(1);  // Empty if no match.

  std::vector<std::string> title_lines = {
    "Halium " + android::base::GetProperty("ro.build.version.release", "(unknown)") +
        " (" + ver_date + ")",
  };
